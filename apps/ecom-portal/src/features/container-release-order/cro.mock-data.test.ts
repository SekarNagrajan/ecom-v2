...
describe('CRO eligibility helper', () => {
  ...
  it('marks expired validity as ineligible', () => {
    ...
    expect(result.eligible).toBe(false);
    expect(result.reasons.some((r) => r.toLowerCase().includes('validity'))).toBe(true);
  });
...
describe('CRO detail seed', () => {
  ...
  it('flips printStatus to Y when document is printed', () => {
    expect(getCROListSnapshot().find((r) => r.croNo === 'CRO-1001')?.printStatus).toBe('N');
    expect(markCROPrinted('CRO-1001')).toBe(true);
    expect(getCROListSnapshot().find((r) => r.croNo === 'CRO-1001')?.printStatus).toBe('Y');
  });
});
