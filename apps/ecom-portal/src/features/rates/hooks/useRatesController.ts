...
export function useRatesController() {
  ...
  // Build combined rate card items
  const cardRates: CombinedRateItem[] = useMemo(() => {
    const items: CombinedRateItem[] = [];
    ...
    return items;
  }, [contracts, tariffs, surcharges]);

  const handleSearch = (params: RateSearchParams) => {
    setSearchParams(params);
    toast.info(`Searching freight rates for ${params.polCode || 'All'} → ${params.podCode || 'All'}...`);
  };

  const handleBookNow = (rate: CombinedRateItem) => {
    toast.success(`Selected rate ${rate.code} (${rate.currency} $${rate.totalEstimatedAmount.toFixed(2)}). Proceeding to Schedules & Booking...`);
  ...
  return {
    ...
    setSearchMode: (mode: RateSearchMode) => setSearchParams((prev) => ({ ...prev, searchMode: mode })),
    cardRates,
    isLoading,
    handleSearch,
    handleBookNow,
