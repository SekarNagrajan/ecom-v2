...
// Register all customer/admin/superuser tokens for /api/auth/me rehydration
for (const entry of Object.values(MOCK_TENANT_USERS)) {
  registerMockToken(entry.token, entry.user);
}
registerMockToken(MOCK_ADMIN_USER.token, MOCK_ADMIN_USER.user);
registerMockToken(MOCK_SUPERUSER_USER.token, MOCK_SUPERUSER_USER.user);
...
export const landingHandlers = [
  ...
  // Tab config (menu access) — handled by auth-admin.handlers.ts
